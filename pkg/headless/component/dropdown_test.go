...
func (d *DropdownTestSuite) TestDropdownFrom() {
	...
	dd := NewDropdown(&v1beta1.Component_Dropdown{
		...
		Merge: []*v1beta1.Component_Dropdown_OptionMerge{
			{
				If: ptr.Ptr("{{ 'Call us' in option.value }}"),
				Option: &v1beta1.Component_Dropdown_Option{
					Hidden: ptr.Ptr(true),
				},
			},
			{
				If: ptr.Ptr(`{{ not ('xxx-' in option.value) }}`),
				Option: &v1beta1.Component_Dropdown_Option{
	...
	c.Store = c.Store.Merge(headless.Store{PageKey: htmlStr})

	mod := dd.Render(c, &Props{})
	actual := mod.(*DropdownModel)

	d.EqualDropdownItems(expectedItems, actual.List.Items())
}
